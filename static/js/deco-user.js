...
var verify = verifyDialogSubmit(
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data.email){
                setTimeout(function(){Dml.fun.showValidateError($('#jsChangeEmail'), data.email);}, 1000);
            }else if(data.code){
                setTimeout(function(){Dml.fun.showValidateError($('#jsChangeEmailCode'), data.code);}, 1000);
            }else if(data.status == "success"){
                //Dml.fun.showErrorTips($('#jsChangeEmailTips'), "邮箱信息更新成功");
                setTimeout(function(){$("#jsChangeEmailTips").html("邮箱信息更新成功").show(500)}, 1000);
    ...
    $('#jsEditUserBtn').on('click', function(){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                }else if(data.cell_number){
                   _showValidateError($('#cell_number'), data.cell_number);
                }else if(data.status == "failure"){
                     Dml.fun.showTipsDialog({
                        title: '保存失败',
                        h2: data.msg
